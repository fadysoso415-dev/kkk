...
export default function MenuViewer({ restaurantSlug, onBackToLanding, lang: initialLang = 'ar' }: MenuViewerProps) {
  ...
  const t = {
    ar: {
      ...
      alertCashier: "تنبيه الكاشير 🛎️",
      alertCashierDesc: "طلب نادل ومساعدة فورية",
      alertPromptTitle: "طلب الخدمة والمساعدة للطاولة 🛎️",
      alertPromptDesc: "الرجاء كتابة رقم طاولتك ليتمكن النادل من تحديد موقعك والتوجه لطاولتك فوراً:",
      alertPromptBtn: "إرسال نداء المساعدة 💬",
      alertPromptPlaceholder: "اكتب رقم الطاولة هنا (مثال: طاولة رقم 4)",
      alertTableRequired: "الرجاء تحديد رقم الطاولة أو الاسم أولاً لطلب المساعدة!"
    },
    en: {
      ...
      alertCashier: "Alert Cashier 🛎️",
      alertCashierDesc: "Request a waiter & help",
      alertPromptTitle: "Table Service & Assistance 🛎️",
      alertPromptDesc: "Please write your table number so the waiter can locate you and come to your table immediately:",
      alertPromptBtn: "Send Assistance Alert 💬",
      alertPromptPlaceholder: "Type table number here (e.g., Table #5)",
      alertTableRequired: "Please specify table number or name first to request assistance!"
    }
  }[lang];
  ...
  const [showTablePrompt, setShowTablePrompt] = useState<boolean>(false);
  const [promptTableNumber, setPromptTableNumber] = useState<string>('');

  const handleAlertCashier = () => {
    const tableToUse = tableNameInput.trim() || promptTableNumber.trim();
    if (!tableToUse) {
      setPromptTableNumber(tableNameInput);
      setShowTablePrompt(true);
      return;
    }

    // Set the table input if it was entered in the prompt but not yet in the main state
    if (!tableNameInput.trim()) {
      setTableNameInput(tableToUse);
    }

    // Generate WhatsApp assistance message
    const restaurantName = lang === 'en' ? (restaurant?.nameEn || restaurant?.name) : restaurant?.name;
    let text = "";
    if (lang === 'en') {
      text += `🛎️ *Table Assistance & Alert Request*\n`;
      text += `• Restaurant: *${restaurantName}*\n`;
      text += `• Table / customer: *${tableToUse}*\n`;
      text += `• Request: Please send a waiter or provide assistance to our table. Thank you! 🙏`;
    } else {
      text += `🛎️ *طلب مساعدة وتنبيه الكاشير*\n`;
      text += `• المطعم: *${restaurantName}*\n`;
      text += `• رقم الطاولة / العميل: *${tableToUse}*\n`;
      text += `• الطلب: نرجو إرسال النادل أو تقديم الخدمة لطاولتنا في أقرب وقت. شكراً لكم! 🙏`;
    }
    ...
    if (!targetPhoneRaw) {
      triggerToast(lang === 'ar' ? 'عذراً، رقم هاتف المطعم غير متاح!' : 'Sorry, restaurant phone number is not available!');
      return;
    }
    ...
    triggerToast(lang === 'ar' 
      ? `🔔 تم إرسال طلب الخدمة والتنبيه للطاولة (${tableToUse})!` 
      : `🔔 Service request & alert sent for Table (${tableToUse})!`);
  ...
  const handleCheckoutWhatsApp = async () => {
    ...
    // If auto inventory linking is enabled, deduct ordered items from the stock
    if (restaurant.enableAutoInventory) {
      try {
        // 1. Local UI instant backup update
        setProducts(prev => prev.map(p => {
          const cartItem = cart.find(ci => ci.product.id === p.id);
          if (cartItem && p.trackInventory) {
            return {
              ...p,
              stockQuantity: Math.max(0, (p.stockQuantity || 0) - cartItem.quantity)
            };
          }
          return p;
        }));

        // 2. Multi-record Firestore Transaction
        const { doc, runTransaction } = await import('firebase/firestore');
        await runTransaction(db, async (transaction) => {
          for (const item of cart) {
            const prodRef = doc(db, 'restaurants', restaurant.id, 'products', item.product.id);
            const prodSnap = await transaction.get(prodRef);
            if (prodSnap.exists()) {
              const pData = prodSnap.data() as Product;
              if (pData.trackInventory) {
                const currentQty = pData.stockQuantity || 0;
                const nextQty = Math.max(0, currentQty - item.quantity);
                transaction.update(prodRef, { stockQuantity: nextQty });
              }
            }
          }
        });
        console.log("Auto-deducted inventory items successfully via transaction.");
      } catch (transactionErr) {
        console.error("Failed to run transaction for inventory deduction:", transactionErr);
      }
    }
  ...
  const matchQuickFilter = (prod: Product) => {
    ...
    if (quickFilter === 'starters' || quickFilter === 'qf1') { // مقبلات
      const keywordsStr = restaurant.qf1Keywords !== undefined ? restaurant.qf1Keywords : 'مقبلات,جانبية,صوص,ثومية,أصابع,بطاطس,سلطة,خبز,fries,appetizer,starter,mozzarella,sauce,garlic,dip';
    ...
    if (quickFilter === 'mains' || quickFilter === 'qf2') { // رئيسي
      const keywordsStr = restaurant.qf2Keywords !== undefined ? restaurant.qf2Keywords : 'برجر,شاورما,وجبة,لحم,دجاج,سندوتش,صاروخ,عربي,burger,shawarma,meal,sandwich,chicken,beef,kebab';
      const keywords = keywordsStr.split(',').map(k => k.trim().toLowerCase()).filter(k => k.length > 0);
      return keywords.some(k => 
        nameLower.includes(k) || 
        descLower.includes(k) || 
    ...
    if (quickFilter === 'desserts' || quickFilter === 'qf3') { // حلويات ومشروبات
      const keywordsStr = restaurant.qf3Keywords !== undefined ? restaurant.qf3Keywords : 'حلو,لوتس,نوتيلا,شوكولاتة,كريب,وافل,مشروب,عصير,شاي,قهوة,كولا,بيبسي,أيس كريم,ميلك شيك,sweet,chocolate,waffle,crepe,cake,dessert,drink,juice,shake,coffee,tea,mojito,pepsi,cola';
      const keywords = keywordsStr.split(',').map(k => k.trim().toLowerCase()).filter(k => k.length > 0);
      return keywords.some(k => 
        nameLower.includes(k) || 
        descLower.includes(k) || 
  ...
  return (
    <div className={`w-full min-h-screen font-sans transition-colors duration-200 ${isDarkMode ? 'dark bg-slate-950 text-slate-100' : 'bg-slate-50 text-slate-900'}`} dir={lang === 'ar' ? 'rtl' : 'ltr'}>
      ...
      <div className="w-full max-w-md mx-auto min-h-screen relative flex flex-col shadow-2xl border-x border-slate-200/50 bg-white dark:bg-slate-900">
            ...
            <div className="shrink-0 flex flex-col gap-1.5 items-end">
              {restaurant.phoneNumber && (
                <a 
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="w-full flex items-center justify-center gap-1.5 px-3 py-1.5 rounded-xl bg-emerald-500 hover:bg-emerald-600 dark:bg-emerald-600 dark:hover:bg-emerald-700 text-white font-extrabold text-[9px] sm:text-[10px] transition-all duration-150 shadow-md shadow-emerald-500/10 hover:scale-105 active:scale-95"
                >
              ...
              <button
                onClick={handleAlertCashier}
                className="w-full flex items-center justify-center gap-1.5 px-3 py-1.5 rounded-xl bg-amber-500 hover:bg-amber-600 text-white font-extrabold text-[9px] sm:text-[10px] transition-all duration-150 shadow-md shadow-amber-500/10 hover:scale-105 active:scale-95 cursor-pointer"
              >
                <Bell className="w-3 h-3 animate-bounce" />
                <span>{t.alertCashier}</span>
              </button>
            </div>
          </div>
        </div>
        ...
        {(() => {
          const showQuickFilters = restaurant.enableQuickFilters !== false;
          const dynamicQuickFilters = [
            {
              id: 'qf1',
              active: restaurant.qf1Active !== false,
              name: restaurant.qf1Name || t.quickAppetizer,
              icon: restaurant.qf1Icon || '🍟'
            },
            {
              id: 'qf2',
              active: restaurant.qf2Active !== false,
              name: restaurant.qf2Name || t.quickMain,
              icon: restaurant.qf2Icon || '🍔'
            },
            {
              id: 'qf3',
              active: restaurant.qf3Active !== false,
              name: restaurant.qf3Name || t.quickDessert,
              icon: restaurant.qf3Icon || '🍰'
            }
          ].filter(f => f.active);

          if (!showQuickFilters || dynamicQuickFilters.length === 0) return null;
          ...
          return (
            <div className="px-4 py-2.5 bg-slate-50/50 dark:bg-slate-900/40 border-b border-slate-100 dark:border-slate-800">
              ...
              <div className="grid gap-2" style={{ gridTemplateColumns: `repeat(${dynamicQuickFilters.length + 1}, minmax(0, 1fr))` }}>
                <button
                  id="qf-all"
                  onClick={() => {
                    setQuickFilter('all');
                ...
                {dynamicQuickFilters.map((qf) => (
                  <button
                    ...
                    style={quickFilter === qf.id ? bgStyle : {}}
                  >
                    <span className="text-base select-none">{qf.icon}</span>
                    <span className="text-[10px] font-black leading-tight line-clamp-1">{qf.name}</span>
                  </button>
                ))}
              </div>
            </div>
        ...
        {/* Floating Alert Cashier button stacked above WhatsApp */}
        {restaurant.phoneNumber && (
          <motion.button
            initial={{ scale: 0, opacity: 0 }}
            animate={{ scale: 1, opacity: 1 }}
            ...
            onClick={handleAlertCashier}
            title={t.alertCashier}
            className={`absolute z-20 flex items-center justify-center gap-1.5 px-3 py-2.5 rounded-full bg-amber-500 hover:bg-amber-600 text-white shadow-xl shadow-amber-500/30 transition-all hover:scale-105 active:scale-95 duration-200 cursor-pointer ${
              cart.length > 0 ? 'bottom-36 left-4' : 'bottom-18 left-4'
            }`}
          >
            <Bell className="w-4 h-4 animate-bounce" />
            <span className="text-[10px] font-black pr-1">{t.alertCashier}</span>
          </motion.button>
        )}
        ...
        {/* Table Number Prompt Modal for Alerting Cashier */}
        {showTablePrompt && (
          <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 shadow-2xl">
            <motion.div
              initial={{ opacity: 0, scale: 0.95 }}
              animate={{ opacity: 1, scale: 1 }}
              className="bg-white dark:bg-slate-900 w-full max-w-sm rounded-3xl shadow-2xl overflow-hidden flex flex-col p-6 border border-slate-100 dark:border-slate-800 text-center"
              dir={lang === 'ar' ? 'rtl' : 'ltr'}
            >
              <div className="w-12 h-12 bg-amber-500/10 rounded-full flex items-center justify-center mx-auto mb-4 text-amber-600">
              ...
              <div className="space-y-4">
                <input
                  type="text"
                  required
                  value={promptTableNumber}
                  onChange={(e) => setPromptTableNumber(e.target.value)}
                  placeholder={t.alertPromptPlaceholder}
                  className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-750 focus:border-amber-500 focus:bg-white dark:focus:bg-slate-950 outline-none rounded-xl p-3 text-xs text-slate-800 dark:text-slate-100 font-extrabold text-center"
                  onKeyDown={(e) => {
                    if (e.key === 'Enter') {
                      e.preventDefault();
                      if (promptTableNumber.trim()) {
                        handleAlertCashier();
                        setShowTablePrompt(false);
                      } else {
                        triggerToast(t.alertTableRequired);
                      }
                    }
                  }}
                />
                ...
                <div className="grid grid-cols-2 gap-2">
                  <button
                    ...
                    className="py-2.5 px-4 rounded-xl border border-slate-200 dark:border-slate-800 text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-850 font-black text-xs transition cursor-pointer"
                  >
                    {t.closeBtn}
                  </button>
                  ...
                  <button
                    ...
                    className="py-2.5 px-4 bg-amber-500 hover:bg-amber-600 text-white font-black rounded-xl text-xs transition cursor-pointer flex items-center justify-center gap-1.5 shadow-md shadow-amber-500/15"
                  >
                    <span>{t.alertPromptBtn}</span>
                  </button>
                </div>
              </div>
            </motion.div>
