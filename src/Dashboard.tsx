...
import { printMenuA4 } from './PrintTemplate';
import { 
  BarChart, 
  Bar, 
  XAxis, 
...
export default function Dashboard({ user, isDemo, onLogout, onNavigateToMenu, lang, onChangeLang }: DashboardProps) {
  ...
  // Delete/Cancel an order entirely before acceptance or during preparation
  const handleDeleteOrder = async (orderId: string) => {
    if (!window.confirm(lang === 'ar' ? 'هل أنت متأكد من حذف وإلغاء هذا الطلب نهائياً؟' : 'Are you sure you want to permanently delete and cancel this order?')) {
      return;
    }
    try {
      const isMockOrder = orderId.startsWith('test-') || orderId.startsWith('demo-');
      if (!isDemo && restaurant.id && restaurant.id !== 'my-restaurant' && !isMockOrder) {
        await deleteOrder(restaurant.id, orderId);
      }
      setOrders(prev => prev.filter(o => o.id !== orderId));
      setAlertMsg({
        type: 'success',
        text: lang === 'ar' ? '🗑️ تم حذف وإلغاء الطلب بنجاح.' : '🗑️ Order successfully deleted and cancelled.'
      });
    } catch (err) {
      console.error(err);
      setAlertMsg({
        type: 'error',
        text: lang === 'ar' ? 'تعذر حذف الطلب، يرجى المحاولة لاحقاً.' : 'Could not delete order, please try again.'
      });
    }
  };
  ...
  const handleUpdateProductFields = async (productId: string, updatedFields: Partial<Product>) => {
    try {
      const existingProduct = products.find(p => p.id === productId);
      if (!existingProduct) return;
      const mergedProduct = { ...existingProduct, ...updatedFields };

      if (isDemo) {
        setProducts(prev => prev.map(p => p.id === productId ? mergedProduct : p));
      } else {
        await saveProduct(restaurant.id, productId, {
          name: mergedProduct.name,
          description: mergedProduct.description || '',
          price: mergedProduct.price,
          originalPrice: mergedProduct.originalPrice || 0,
          isDiscounted: mergedProduct.isDiscounted || false,
          discountPrice: mergedProduct.discountPrice || 0,
          image: mergedProduct.image,
          categoryId: mergedProduct.categoryId,
          isAvailable: mergedProduct.isAvailable,
          badge: mergedProduct.badge || '',
          order: mergedProduct.order,
          discountLabel: mergedProduct.discountLabel || '',
          discountExpiry: mergedProduct.discountExpiry || '',
          sizes: mergedProduct.sizes || [],
          trackInventory: mergedProduct.trackInventory || false,
          stockQuantity: mergedProduct.stockQuantity !== undefined ? mergedProduct.stockQuantity : 0,
          alertLowStock: mergedProduct.alertLowStock !== undefined ? mergedProduct.alertLowStock : 5
        });
        setProducts(prev => prev.map(p => p.id === productId ? mergedProduct : p));
      }
      triggerAlert('success', lang === 'ar' ? 'تم تحديث بيانات المخزون بنجاح!' : 'Inventory updated successfully!');
    } catch (err: any) {
      console.error(err);
      triggerAlert('error', lang === 'ar' ? 'فشل تحديث بيانات المنتج.' : 'Failed to update product.');
    }
  };

  const handleUpdateRestaurantFields = async (updatedFields: Partial<Restaurant>) => {
    try {
      const mergedRest = { ...restaurant, ...updatedFields };
      if (isDemo) {
        setRestaurant(mergedRest);
      } else {
        await saveRestaurant(restaurant.id, mergedRest);
        setRestaurant(mergedRest);
      }
      triggerAlert('success', lang === 'ar' ? 'تم حفظ التحديثات بنجاح!' : 'Settings updated successfully!');
    } catch (err: any) {
      console.error(err);
      triggerAlert('error', lang === 'ar' ? 'فشل حفظ الإعدادات.' : 'Failed to update settings.');
    }
  };

  const deductInventoryForOrder = async (items: Array<{ productId: string, quantity: number }>) => {
    if (!restaurant.enableAutoInventory) return;
    try {
      for (const item of items) {
        const prod = products.find(p => p.id === item.productId);
        if (prod && prod.trackInventory) {
          const nextQty = Math.max(0, (prod.stockQuantity || 0) - item.quantity);
          
          // 1. Local update
          setProducts(prev => prev.map(p => p.id === prod.id ? { ...p, stockQuantity: nextQty } : p));
          
          // 2. Firebase update if not in demo
          if (!isDemo) {
            await saveProduct(restaurant.id, prod.id, {
              name: prod.name,
              description: prod.description || '',
              price: prod.price,
              ...
              alertLowStock: prod.alertLowStock !== undefined ? prod.alertLowStock : 5
            });
          }
        }
      }
      console.log("Central automatic stock deduction completed successfully.");
    } catch (err) {
      console.error("Error deducting inventory for order items: ", err);
    }
  };
        ...
        <div className="space-y-8">
          ...
          <nav className="flex flex-col gap-1.5">
            ...
            <button
              ...
              className={`w-full py-3 px-4 rounded-xl font-bold text-xs sm:text-sm flex items-center gap-3 cursor-pointer transition duration-150 ${activeTab === 'inventory' ? 'bg-amber-500 text-white shadow-lg shadow-amber-500/15' : 'hover:bg-slate-800 hover:text-white'}`}
            >
              <Package className="w-4.5 h-4.5" />
              إدارة المخزون والمستودع 📦
              {products.filter(p => p.trackInventory && (p.stockQuantity || 0) <= (p.alertLowStock || 5)).length > 0 && (
                <span className="mr-auto bg-rose-600 text-[10px] text-white px-2 py-0.5 rounded-full font-sans font-extrabold animate-pulse">
                  {products.filter(p => p.trackInventory && (p.stockQuantity || 0) <= (p.alertLowStock || 5)).length}
                </span>
              )}
            </button>
      ...
      <main className="flex-1 p-4 md:p-8 pb-24 md:pb-8 space-y-8 overflow-y-auto max-w-5xl">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-slate-200 pb-5">
          <div>
            <h3 className="text-xl sm:text-2xl font-black text-slate-900">
              ...
              {activeTab === 'inventory' && 'نظام إدارة المخزون والمستودع 📦'}
        ...
        {activeTab === 'inventory' && (
          <InventoryManager
            ...
            onUpdateProduct={handleUpdateProductFields}
            onUpdateRestaurant={handleUpdateRestaurantFields}
            isDemo={isDemo}
            lang={lang}
          />
        )}
        ...
        {activeTab === 'pos' && (
          <CashierPOS
            ...
            onSaveOrder={async (orderData) => {
              // Automatically deduct stock levels if auto inventory linking is enabled
        ...
        {activeTab === 'settings' && (
          <form onSubmit={handleSaveSettings} className="bg-white border border-slate-100 p-6 md:p-8 rounded-3xl shadow-md space-y-6">
            ...
            {/* نظام الفلاتر السريعة المخصص */}
            <div className="space-y-4 font-sans text-right" dir="rtl">
              <div className="flex items-center gap-2">
                <Tag className="w-5 h-5 text-amber-500 shrink-0" />
                <h4 className="text-sm font-black text-slate-900 font-sans">تخصيص الفلاتر السريعة بالمنيو 🏷️⚡</h4>
              </div>
              <p className="text-[11px] text-slate-500 leading-relaxed font-sans">
                هنا يمكنك تفعيل أو إخفاء أزرار الفلترة السريعة أعلى المنتجات بالمنيو، وتعديل أيقوناتها (الإيموجي) وأسمائها لتناسب نشاطك التجاري بالكامل (كافيه، حلويات، ملابس، إلخ). يمكنك أيضاً تحديد الكلمات المفتاحية لفلترة المنتجات تلقائياً بناءً عليها.
              </p>

              <div className="bg-slate-50 p-4.5 rounded-3xl border border-slate-100 space-y-4">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold text-slate-800 font-sans">تفعيل شريط الفلترة السريعة بالمنيو 🚀</span>
                  <label className="relative inline-flex items-center cursor-pointer select-none">
                    <input 
                      type="checkbox" 
                      checked={restaurant.enableQuickFilters !== false} 
                      onChange={e => setRestaurant({ ...restaurant, enableQuickFilters: e.target.checked })}
                ...
                {restaurant.enableQuickFilters !== false && (
                  <div className="space-y-4 pt-2 border-t border-slate-200/50">
                    ...
                    <div className="p-4 bg-white border border-slate-100 rounded-2xl space-y-3 shadow-sm">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <span className="text-xs bg-amber-100 text-amber-800 px-2 py-0.5 rounded font-black">فلتر 1</span>
                          <span className="text-xs font-bold text-slate-800">الفلتر الأول</span>
                        </div>
                        <label className="flex items-center gap-1.5 cursor-pointer text-[11px] font-bold text-slate-600">
                          <input 
                            type="checkbox" 
                            checked={restaurant.qf1Active !== false}
                            onChange={e => setRestaurant({ ...restaurant, qf1Active: e.target.checked })}
                      ...
                      {restaurant.qf1Active !== false && (
                        <div className="grid sm:grid-cols-3 gap-3">
                          <div>
                            <label className="block text-[10px] font-bold text-slate-600 mb-1">اسم الفلتر</label>
                            <input 
                              type="text" 
                              value={restaurant.qf1Name !== undefined ? restaurant.qf1Name : 'مقبلات'} 
                              onChange={e => setRestaurant({ ...restaurant, qf1Name: e.target.value })}
                              className="w-full bg-slate-50 border border-slate-200 rounded-xl p-2.5 text-xs outline-none focus:bg-white focus:border-amber-500"
                              placeholder="مثال: مقبلات"
                            />
                          </div>
                          <div>
                            <label className="block text-[10px] font-bold text-slate-600 mb-1">أيقونة الفلتر (إيموجي)</label>
                            <input 
                              type="text" 
                              value={restaurant.qf1Icon !== undefined ? restaurant.qf1Icon : '🍟'} 
                              onChange={e => setRestaurant({ ...restaurant, qf1Icon: e.target.value })}
                              className="w-full bg-slate-50 border border-slate-200 rounded-xl p-2.5 text-xs outline-none focus:bg-white focus:border-amber-500 text-center"
                              placeholder="🍟"
                            />
                          </div>
                          <div className="sm:col-span-1">
                            <label className="block text-[10px] font-bold text-slate-600 mb-1">كلمات الفلترة الدلالية (مفصولة بفاصلة)</label>
                            <input 
                              type="text" 
                              value={restaurant.qf1Keywords !== undefined ? restaurant.qf1Keywords : 'مقبلات,جانبية,صوص,ثومية,أصابع,بطاطس,سلطة,خبز,fries,appetizer,starter,mozzarella,sauce,garlic,dip'} 
                              onChange={e => setRestaurant({ ...restaurant, qf1Keywords: e.target.value })}
                              className="w-full bg-slate-50 border border-slate-200 rounded-xl p-2.5 text-xs outline-none focus:bg-white focus:border-amber-500"
                              placeholder="مقبلات, بطاطس, صوص"
                            />
                          </div>
                        </div>
                      )}
                    ...
                    <div className="p-4 bg-white border border-slate-100 rounded-2xl space-y-3 shadow-sm">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <span className="text-xs bg-amber-100 text-amber-800 px-2 py-0.5 rounded font-black">فلتر 2</span>
                          <span className="text-xs font-bold text-slate-800 font-sans">الفلتر الثاني</span>
                        </div>
                        <label className="flex items-center gap-1.5 cursor-pointer text-[11px] font-bold text-slate-600">
                          <input 
                            type="checkbox" 
                            checked={restaurant.qf2Active !== false}
                            onChange={e => setRestaurant({ ...restaurant, qf2Active: e.target.checked })}
                            className="rounded text-amber-500 focus:ring-amber-500"
                          />
                          <span>نشط ومفعّل</span>
                        </label>
                      ...
                      {restaurant.qf2Active !== false && (
                        <div className="grid sm:grid-cols-3 gap-3">
                          <div>
                            <label className="block text-[10px] font-bold text-slate-600 mb-1">اسم الفلتر</label>
                            <input 
                              type="text" 
                              value={restaurant.qf2Name !== undefined ? restaurant.qf2Name : 'الرئيسية'} 
                              onChange={e => setRestaurant({ ...restaurant, qf2Name: e.target.value })}
                              className="w-full bg-slate-50 border border-slate-200 rounded-xl p-2.5 text-xs outline-none focus:bg-white focus:border-amber-500"
                              placeholder="مثال: رئيسي"
                            />
                          </div>
                          <div>
                            <label className="block text-[10px] font-bold text-slate-600 mb-1">أيقونة الفلتر (إيموجي)</label>
                            <input 
                              type="text" 
                              value={restaurant.qf2Icon !== undefined ? restaurant.qf2Icon : '🍔'} 
                              onChange={e => setRestaurant({ ...restaurant, qf2Icon: e.target.value })}
                              className="w-full bg-slate-50 border border-slate-200 rounded-xl p-2.5 text-xs outline-none focus:bg-white focus:border-amber-500 text-center"
                              placeholder="🍔"
                            />
                          </div>
                          <div className="sm:col-span-1">
                            ...
                            <input 
                              type="text" 
                              value={restaurant.qf2Keywords !== undefined ? restaurant.qf2Keywords : 'برجر,شاورما,وجبة,لحم,دجاج,سندوتش,صاروخ,عربي,burger,shawarma,meal,sandwich,chicken,beef,kebab'} 
                              onChange={e => setRestaurant({ ...restaurant, qf2Keywords: e.target.value })}
                              className="w-full bg-slate-50 border border-slate-200 rounded-xl p-2.5 text-xs outline-none focus:bg-white focus:border-amber-500"
                              placeholder="برجر, دجاج, لحم"
                            />
                          </div>
                        </div>
                      )}
                    ...
                    <div className="p-4 bg-white border border-slate-100 rounded-2xl space-y-3 shadow-sm">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <span className="text-xs bg-amber-100 text-amber-800 px-2 py-0.5 rounded font-black">فلتر 3</span>
                          <span className="text-xs font-bold text-slate-800 font-sans">الفلتر الثالث</span>
                        </div>
                        <label className="flex items-center gap-1.5 cursor-pointer text-[11px] font-bold text-slate-600">
                          <input 
                            type="checkbox" 
                            checked={restaurant.qf3Active !== false}
                            onChange={e => setRestaurant({ ...restaurant, qf3Active: e.target.checked })}
                            className="rounded text-amber-500 focus:ring-amber-500"
                          />
                          <span>نشط ومفعّل</span>
                        </label>
                      ...
                      {restaurant.qf3Active !== false && (
                        <div className="grid sm:grid-cols-3 gap-3">
                          <div>
                            <label className="block text-[10px] font-bold text-slate-600 mb-1">اسم الفلتر</label>
                            <input 
                              type="text" 
                              value={restaurant.qf3Name !== undefined ? restaurant.qf3Name : 'حلويات'} 
                              onChange={e => setRestaurant({ ...restaurant, qf3Name: e.target.value })}
                              className="w-full bg-slate-50 border border-slate-200 rounded-xl p-2.5 text-xs outline-none focus:bg-white focus:border-amber-500"
                          ...
                          <div>
                            ...
                            <input 
                              type="text" 
                              value={restaurant.qf3Icon !== undefined ? restaurant.qf3Icon : '🍰'} 
                              onChange={e => setRestaurant({ ...restaurant, qf3Icon: e.target.value })}
                              className="w-full bg-slate-50 border border-slate-200 rounded-xl p-2.5 text-xs outline-none focus:bg-white focus:border-amber-500 text-center"
                              placeholder="🍰"
                            />
                          </div>
                          <div className="sm:col-span-1">
                            ...
                            <input 
                              type="text" 
                              value={restaurant.qf3Keywords !== undefined ? restaurant.qf3Keywords : 'حلو,لوتس,نوتيلا,شوكولاتة,كريب,وافل,مشروب,عصير,شاي,قهوة,كولا,بيبسي,أيس كريم,ميلك شيك,sweet,chocolate,waffle,crepe,cake,dessert,drink,juice,shake,coffee,tea,mojito,pepsi,cola'} 
                              onChange={e => setRestaurant({ ...restaurant, qf3Keywords: e.target.value })}
                              className="w-full bg-slate-50 border border-slate-200 rounded-xl p-2.5 text-xs outline-none focus:bg-white focus:border-amber-500"
                              placeholder="حلو, كريب, قهوة, شاي"
                            />
                          </div>
                        </div>
                      )}
          ...
          <div className="space-y-6">
            ...
            <div className="flex items-center justify-between flex-wrap gap-4">
              ...
              <div className="flex items-center gap-2">
                <button
                  type="button"
                  onClick={() => printMenuA4(restaurant, categories, products, { lang })}
                  className="bg-indigo-50 hover:bg-indigo-100 text-indigo-700 border border-indigo-200 font-bold py-2.5 px-4 rounded-xl text-xs sm:text-sm transition flex items-center gap-2 cursor-pointer shadow-sm"
                  title="طباعة وتحميل قائمة الطعام كاملة كملف PDF"
                >
                  <Printer className="w-4 h-4" />
                  {lang === 'ar' ? 'طباعة المنيو (PDF) 🖨️' : 'Print Menu (PDF) 🖨️'}
                </button>
                <button
                  onClick={() => {
            ...
            <div className="space-y-4">
              ...
              <div className="pt-4 flex flex-wrap gap-2.5">
                <button
                  type="button"
                  onClick={() => printMenuA4(restaurant, categories, products, { lang })}
                  className="bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-2.5 px-4 rounded-xl text-xs transition cursor-pointer flex items-center gap-1.5"
                  title="تحميل وطباعة المنيو كامل بجودة عالية A4"
                >
                  <Printer className="w-4 h-4" />
                  {lang === 'ar' ? 'طباعة المنيو كامل (PDF) 🖨️' : 'Print Full Menu (PDF) 🖨️'}
                </button>
        ...
        {activeTab === 'orders' && (() => {
          const activePrepOrders = orders.filter(o => o.status === 'pending' || o.status === 'preparing' || !o.status);
          const pendingCount = orders.filter(o => o.status === 'pending' || !o.status).length;
          const preparingCount = orders.filter(o => o.status === 'preparing').length;

          const displayedInTab = activePrepOrders.filter(o => {
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="bg-rose-500/10 border border-rose-500/20 p-4 rounded-2xl flex flex-col justify-between">
                  <div>
                    <span className="text-xs text-rose-700 font-bold block">إجمالي المطلوب تحضيره 🛎️</span>
                    <h2 className="text-2xl font-black text-rose-850 mt-2">{activePrepOrders.length}</h2>
                  </div>
                  <button 
                    onClick={injectSampleOrderForTesting}
                    className="text-[10px] text-white bg-rose-600 hover:bg-rose-750 px-3 py-1.5 rounded-xl shrink-0 w-fit transition mt-2 font-black cursor-pointer shadow-sm flex items-center gap-1 self-start"
                  >
                    <span>🧪 محاكاة طلب تحضير جديد</span>
                  </button>
                </div>
              ...
              <div className="space-y-4">
                ...
                <div className="flex flex-wrap items-center gap-1.5 border-b border-slate-200 pb-3">
                  ...
                  >
                    الكل المطلوب تحضيره ({activePrepOrders.length})
                  </button>
                  <button
                              ...
                              {isPending && (
                                <div className="flex gap-1">
                                  <button
                                    ...
                                    className="bg-rose-50 hover:bg-rose-100 text-rose-600 border border-rose-200 text-[10px] py-1.5 px-2.5 rounded-lg font-bold cursor-pointer transition whitespace-nowrap"
                                    title="حذف وإلغاء الطلب 🗑️"
                                  >
                                    🗑️ إلغاء الطلب
                                  </button>
                                  <button
                                  ...
                                  <div className="flex items-center justify-center gap-2">
                                    {isPending && (
                                      <button
                                        ...
                                        className="text-rose-500 hover:text-rose-700 font-bold hover:underline transition cursor-pointer text-[10.5px] border border-rose-200/50 bg-rose-50/50 px-2.5 py-1 rounded-lg"
                                        title="حذف وإلغاء الطلب 🗑️"
                                      >
                                        إلغاء وحذف 🗑️
                                      </button>
